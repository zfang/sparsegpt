...
def get_bloom(model, use_safetensors):
    import torch
    ...
    model = BloomForCausalLM.from_pretrained(model, torch_dtype='auto', use_safetensors=use_safetensors)
    model.seqlen = 2048
    return model


@torch.no_grad()
def bloom_sequential(model, dataloader, dev, args):
    print('Starting ...')
...
@torch.no_grad()
def bloom_eval(model, testenc, dev, dataset: str, args: object):
    print('Evaluation...')
...
if __name__ == '__main__':
    ...
    model = get_bloom(args.model, args.use_safetensors)
    model.eval()
    ...
    for dataset in ['wikitext2', 'ptb', 'c4']:
        ...
        bloom_eval(model, testloader, DEV, dataset, args)
